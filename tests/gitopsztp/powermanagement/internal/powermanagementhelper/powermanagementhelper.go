...
func WaitForPodsHealthy(pods []*pod.Builder, timeout time.Duration) {
	Eventually(func() error {
		for _, singlePod := range pods {
			tempPod, err := pod.Pull(gitopsztpinittools.HubAPIClient, singlePod.Definition.Name,
...
func DefineStressPod(nodeName string, cpus int, guaranteed bool) *pod.Builder {
	...
	pod := pod.NewBuilder(gitopsztpinittools.HubAPIClient, "", powermanagementparams.NamespaceTesting, stressngImage)
