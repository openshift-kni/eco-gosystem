...
func (builder *Builder) WithReadOnlyRootFilesystem(readOnlyRootFilesystem bool) *Builder {
	...
	builder.Definition.AllowHostPorts = readOnlyRootFilesystem
...
func (builder *Builder) WithFSGroup(fsGroupType string, fsGroupRange map[string]int64) *Builder {
	if valid, _ := builder.validate(); !valid {
		return builder
	}

	glog.V(100).Infof("Redefining SecurityContextConstraints %s with"+
		"fsGroupType: %s", builder.Definition.Name, fsGroupType)

	if fsGroupType == "" {
		glog.V(100).Infof("SecurityContextConstraints 'fsGroupType' argument cannot be empty")
	...
	builder.Definition.FSGroup.Type = securityV1.FSGroupStrategyType(fsGroupType)

	if fsGroupRange == nil {
		glog.V(100).Infof("SecurityContextConstraints 'fsGroupRange' argument is empty")

		return builder
	}

	var rangeValues securityV1.IDRange
	rangeValues.Max = fsGroupRange["max"]
	rangeValues.Min = fsGroupRange["min"]

	builder.Definition.FSGroup.Ranges = make([]securityV1.IDRange, 1)
	builder.Definition.FSGroup.Ranges = append(builder.Definition.FSGroup.Ranges, rangeValues)
...
func (builder *Builder) WithGroups(groups []string) *Builder {
	...
	glog.V(100).Infof("Redefining SecurityContextConstraints %s with"+
		"groups: %s", builder.Definition.Name, groups)
